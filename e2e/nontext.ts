 ...
 *
 * BOTH HALVES ARE LIVE IN THIS LAB. `styles/main.css` has the boundary token
 * — `--border-strong`, one definition because dark is the only theme here —
 * and applies it to every `input`, `textarea` and `select`, to `#app button`,
 * and to the two non-control surfaces that borrow the look (`.mono-out`, the
 * capacity track). But tokens are discarded by overrides, and that is what
 * this oracle exists to measure rather than trust. Four shapes on this page
 * override or bypass it: `#app button.primary` — Encode and Decode — repaints
 * its border the SAME colour as its own accent fill, so it has no edge of its
 * own and lives or dies by fill-vs-surround, which here is `--accent` on the
 * panel's `--surface` at about 5.8:1 and lightens further on hover;
 * `#app button:hover` swaps the edge to `--accent`; `[aria-invalid="true"]`
 * recolours it to `--danger` on the pattern box, the length box and the salt
 * box, states only the drive reaches; and `#app button:disabled` drops to the
 * decorative `--border`, which 1.4.11 exempts as an inactive component and the
 * walk below skips for exactly that reason. The shared bar's `<a class="cl-btn">` links
 * are controls too, with a `color-mix()` edge and a translucent hover fill.
 * This oracle judges each of them as painted, at every driven state.
 *
 * The generated-content half has one tenant, and it is a WCAG 1.4.1 fix rather
 * than decoration: `tbody tr.is-chosen td:first-child::before` paints a "▸" in
 * `--accent-soft` so the chosen count-table row is marked by a glyph and not
 * by its accent fill alone. `--accent` itself measures 4.33:1 over that fill,
 * under the 4.5:1 the glyph needs, which is exactly the kind of margin no
 * other oracle on this page can see. The `<summary>` disclosure triangles are
 * `::marker`, the UA's own, and are out of reach of both.
 *
 ...
 * background beneath — because a control's fill is often an `rgba()` or a
 * `color-mix()`, and reading the declared value would measure something the
 * page never paints. Here that is the primary button's hover fill, the shared
 * bar's translucent hover fill, and the accent wash `<body>` paints under
 * everything that is not sitting on an opaque panel.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * data-URI chevron is one, and is passed over. This lab DOES declare
     * gradients: `<body>` washes two `radial-gradient`s of `--accent` and
     * `--accent-2` over `--bg`. They do not currently reach a judgement here,
     * because every control on this page sits inside an opaque `.panel` or the
     * opaque top bar and the backdrop walk stops at the first opaque ancestor
     * — `contrast.ts`, which measures the hero and footer text that sits
     * directly on `<body>`, does exercise them. The sampler is the fleet's
     * shared paint model and stays whole: the day a control is placed outside
     * a panel, or a panel goes translucent, it is measured on the wash rather
     * than against a surface that is not there.
     */
     ...
     *
     * A driven pass here walks eight panels, and the expensive part is the
     * generated-content sweep: it asks every element in `body *` for two
     * pseudo-styles, which on this page means up to 400 transition rows of
     * five cells apiece plus the count table's own rows — all siblings
     ...
     * measuring the theme, and this lab has only the one theme to be wrong
     * about. The shape is closer here than it looks: this page's canvas IS two
     * radial gradients. What saves it is that `html` and `body` each also
     * declare an opaque `background-color: var(--bg)` LONGHAND underneath the
     * wash, so the root resolves on the first try and the `<body>` fallback is
     * dormant. Both halves stay because either declaration is one refactor
     * away from vanishing. The gradient-aware sampler is borrowed from
     * `contrast.ts` wholesale rather than approximated a second time.
     */
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // Every control this lab paints — the inputs, the textareas, the preset
      // select, `#app button` and the shared bar's `.cl-btn` links — carries
      // all four sides today, so the per-side walk changes no number here; it
      // is what keeps that an implementation detail rather than a load-bearing
      // assumption. The page does use single-side rules elsewhere (the
      // `.callout`'s 3px accent left edge, the `.limit` and trace-row top
      // rules), none of them on a control, which is how the one-sided
      // delineator arrives in a stylesheet in the first place.
      //
      ...
      // stylesheet declares no transparent border today, so the alpha test
      // excludes nothing here. It stays because that spacer is the standard
      // way to reserve room for a state colour, and counting one as a border
      // reports 1.00:1 for a control that is in fact identified by its text
      // alone — exactly the case the "is it trying to draw itself as a
      ...
      //
      // Nothing here is exempted by it today: this lab ships text, password
      // and number inputs, two textareas and one select, and none of those is
      // in the list. The select is the near miss — a UA-drawn dropdown would
      // qualify on look, but `styles/main.css` takes it over with
      // `appearance: none` and paints its own chevron, so it is judged in
      // full. The test stays for the first checkbox or range a settings panel
      // adds.
      ...
      for (const side of paintedSides) {
        ...
        // rgb(115,97,0) a screenshot of the page actually contains. Every edge
        // colour on this page is opaque, so the composite is a no-op for the
        // border here; the FILL above is where translucency is live, since the
        // shared bar's `.cl-btn:hover` fills with
        // `color-mix(in srgb, var(--accent) 14%, transparent)` over the bar.
